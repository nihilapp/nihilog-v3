...
interface UseGetUserSubscribeListOptions extends QueryOptionType<ListType<SelectUserSubscribeInfoListItemType>> {}

/**
 * @description 전체 사용자 구독 설정 목록을 조회하는 커스텀 훅
 * @param {SearchSubscribeType} searchData - 검색 데이터
 * @param {UseGetUserSubscribeListOptions} [options] - 쿼리 옵션 (선택사항)
 */
export function useGetUserSubscribeList(
  searchData: SearchSubscribeType,
  options: UseGetUserSubscribeListOptions = {}
) {
