...
interface UseAdminGetTagMappingsOptions extends QueryOptionType<ListType<SelectPostTagMappingListItemType>> {
  searchParams?: SearchPstTagMpngType;
}

/**
 * @description 관리자용 태그 매핑 검색을 위한 커스텀 훅 (POST 방식)
 * @param {UseAdminGetTagMappingsOptions} [options] - 쿼리 옵션 (선택사항)
 * @returns 태그 매핑 검색 쿼리 객체
 */
export function useAdminGetTagMappings(options: UseAdminGetTagMappingsOptions = {}) {
  const { searchParams = {}, ...queryOptions } = options;

  const query = usePostQuery<ListType<SelectPostTagMappingListItemType>, SearchPstTagMpngType>({
    url: [
      'admin', 'tags', 'mapping', 'search',
    ],
    key: adminTagsKeys.searchMappings(searchParams),
    body: searchParams,
    callback() {
