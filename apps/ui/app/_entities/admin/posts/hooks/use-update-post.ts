...
interface UseUpdatePostOptions extends MutationOptionsType<SelectPostType, UpdatePostType> {
  pstNo: number;
}

/**
 * @description 포스트를 수정하는 커스텀 훅
 * @param {UseUpdatePostOptions} [options] - 뮤테이션 옵션 (선택사항)
 */
export function useUpdatePost(options: UseUpdatePostOptions = { pstNo: 0, }) {
